...
export function ValidatePostSharing(validationOptions?: ValidationOptions) {
  return ValidateDependentFields(
    ...
    (args) => {
      ...
      if (shareAsPost && !post)
        return 'Post is required when shareAsPost is true!';
      if (shareAsPost && !isPublic)
        return 'Event must be public if shareAsPost is true!';
      if (post && !shareAsPost)
        return 'shareAsPost must be true if post is provided!';
